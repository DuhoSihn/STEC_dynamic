function [ W_f, b_f, LS ] = STE2_learn_sgm_GPU( X, info_X_sX, io_f, hl_f, lambda, kWidth, cType, miniBatchSize, nPred, nMaxIter, varargin )
...
% info_X_sX : the original size of images of X and sX and the shift of sX in X, (3) X (2) matrix.
% info_X_sX( 1, [ 1, 2 ] ) = the original size of X, [ 1st Dim., 2nd Dim. ].
% info_X_sX( 2, [ 1, 2 ] ) = the original size of sX, [ 1st Dim., 2nd Dim. ].
% info_X_sX( 3, [ 1, 2 ] ) = the shift range of sX in X, [ Min., Max. ], non-negative integers corresponding to moving pixels.
...
% lambda : balancing paramter, (the number of hierarchies) X (1) vector.
% lambda( h ) : balance of the first to second expectation.
% lambda( h ) > 0.
% kWidth : kernel widths used in kernel density estimation to maximize entropy, (the number of hierarchy) X (1) vector.
% cType : coding type, (the number of hierarchy) X (1) cell array.
% cType( h ) : 'regular' or 'sparse'.
...
% nPred : [ movement iterations, fixation iterations ], (1) X (2) vector.
% nMaxIter : the number of maximum allowable iterations of the gradient method, scalar.
%
% W_f{ h, 1 } : weights of the neural network f, (the number of hidden layers + 1) X (2) cell.
% W_f{ h, 1 } : bottom-up.
% W_f{ h, 2 } : top-down.
...
% e.g.) [ W_f, b_f, LS ] = STE2_learn_sgm( X, info_X_sX, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter, 'initial', W_f, b_f )
% params : learning parameters of Adam optimization.
% e.g.) [ W_f, b_f, LS ] = STE2_learn_sgm( X, info_X_sX, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter, 'params', alpha, beta1, beta2, epsilon )
...
for h = 1 : size( io_f, 1 )
    for d = 1 : 2
        for pi = 1 : size( io_f{ h, d }, 1 )
            for po = 1 : 2
                io_f{ h, d }{ pi, po } = gpuArray( io_f{ h, d }{ pi, po } );
            end
        end
    end
end
for h = 1 : size( hl_f, 1 )
    for d = 1 : 2
        for p = 1 : size( hl_f{ h, d } )
            hl_f{ h, d }{ p, 1 } = gpuArray( hl_f{ h, d }{ p, 1 } );
        end
    end
end
lambda = gpuArray( lambda );
kWidth = gpuArray( kWidth );
miniBatchSize = gpuArray( miniBatchSize );
nPred = gpuArray( nPred );
nMaxIter = gpuArray( nMaxIter );
...
for h = 1 : size( W_f, 1 )
    for d = 1 : 3
        ...
        for p = 1 : nPh
            for l = 1 : size( W_f{ h, d }{ p, 1 }, 1 )
                W_f{ h, d }{ p, 1 }{ l, 1 } = gpuArray( W_f{ h, d }{ p, 1 }{ l, 1 } );
                b_f{ h, d }{ p, 1 }{ l, 1 } = gpuArray( b_f{ h, d }{ p, 1 }{ l, 1 } );
            end
        end
    end
end
...
for h = 1 : nH
    for d = 1 : 3
        ...
        for p = 1 : nPh
            ...
            for l = 1 : nL_f( h, d, p )
                m_W_f{ h, d }{ p, 1 }{ l, 1 } = zeros( size( W_f{ h, d }{ p, 1 }{ l, 1 } ), 'gpuArray' );
                m_b_f{ h, d }{ p, 1 }{ l, 1 } = zeros( size( b_f{ h, d }{ p, 1 }{ l, 1 } ), 'gpuArray' );
                v_W_f{ h, d }{ p, 1 }{ l, 1 } = zeros( size( W_f{ h, d }{ p, 1 }{ l, 1 } ), 'gpuArray' );
                v_b_f{ h, d }{ p, 1 }{ l, 1 } = zeros( size( b_f{ h, d }{ p, 1 }{ l, 1 } ), 'gpuArray' );
            end
        end
    end
end
...
% -------------------------------------------------------------------------
LS{ 1, 1 } = NaN( nMaxIter, nH + 1, 'gpuArray' );
LS{ 2, 1 } = NaN( nMaxIter, nH, 'gpuArray' );
% -------------------------------------------------------------------------
...
while stopCriterion == 0
    ...
    [ sX, sX_learn ] = STE2_gaze_GPU( sX, info_X_sX, nPred );
    
    if t == 1
        [ Z2, E2, Z1, ~, Z0, oZ1, oZ2 ] = STE_pred_sgm_GPU( sX, io_f, W_f, b_f, 'learn' );
    else
        for h = 1 : nH
            rX{ h, 1 } = Z2{ h, 1 }( ( prod( nPred, 2 ) - 3 ) * miniBatchSize + [ 1 : miniBatchSize ], : );
        end
        [ Z2, E2, Z1, ~, Z0, oZ1, oZ2 ] = STE_pred_sgm_GPU( sX, io_f, W_f, b_f, 'learn', 'prior', rX );
    end
    ...
    for h = 1 : nH
        if strcmpi( cType{ h, 1 }, 'regular' )
            sZ1{ h, 1 } = rand( miniBatchSize * ( prod( nPred, 2 ) - 2 ), nZ( h ), 'gpuArray' );
        elseif strcmpi( cType{ h, 1 }, 'sparse' )
            sZ1{ h, 1 } = ( 2 * rand( miniBatchSize * ( prod( nPred, 2 ) - 2 ), nZ( h ), 'gpuArray' ) ) - 1;
        end
    end
    ...
    for h = 1 : nH
        for d = 1 : 3
            ...
            ct{ h, d } = cell( nPh, 1 );
            for p = 1 : nPh
                ct{ h, d }{ p, 1 } = zeros( nL_f( h, d, p ), 1, 'gpuArray' );
            end
        end
    end
    ...
    for h = 1 : nH
        ...
        for d2 = [ 1, 2, 3 ]% (2) bottom-up, top-down, recurrent
            ...
            if d2 == 1
                if h == 1
                    d_cum2_e = zeros( nV, ( prod( nPred, 2 ) - 2 ) * miniBatchSize, 'gpuArray' );
                else
                    d_cum2_e = zeros( nZ( h - 1 ), ( prod( nPred, 2 ) - 2 ) * miniBatchSize, 'gpuArray' );
                end
            elseif d2 == 2
                d_cum2_e = zeros( nZ( h ), ( prod( nPred, 2 ) - 2 ) * miniBatchSize, 'gpuArray' );
            elseif d2 == 3
                d_cum2_e = zeros( nZ( h ), ( prod( nPred, 2 ) - 2 ) * miniBatchSize, 'gpuArray' );
            end
        ...
        for d = 1 : 3
            ...
            for p = 1 : nPh
                ...
                for l = 1 : nL_f( h, d, p )
                    ...
                    if ~( h == 1 && d == 2 )
                        dW_f_t = sum( dW_f_1{ h, d }{ p, 1 }{ l, 1 }, 3 ) + lambda( h ) * dW_f_2{ h, d }{ p, 1 }{ l, 1 };% gradient for weight in Equation ()
                        db_f_t = sum( db_f_1{ h, d }{ p, 1 }{ l, 1 }, 2 ) + lambda( h ) * db_f_2{ h, d }{ p, 1 }{ l, 1 };% gradient for bias in Equation ()
                    else
    ...
    for h = 1 : nH
        if h < nH
            ...
            t_db_f_t = zeros( nZ( h ), 3, 'gpuArray' );
        ...
        for d = 1 : 3
            ...
            for p = 1 : nPh
                for l = 1 : nL_f( h, d, p )
                    ...
                    W_f{ h, d }{ p, 1 }{ l, 1 } = W_f{ h, d }{ p, 1 }{ l, 1 } - alpha * ( m_W_f_hat ./ ( sqrt( v_W_f_hat ) + epsilon * ones( size( v_W_f_hat ), 'gpuArray' ) ) );% gradient descent
                    b_f{ h, d }{ p, 1 }{ l, 1 } = b_f{ h, d }{ p, 1 }{ l, 1 } - alpha * ( m_b_f_hat ./ ( sqrt( v_b_f_hat ) + epsilon * ones( size( v_b_f_hat ), 'gpuArray' ) ) );% gradient descent
...
for h = 1 : size( W_f, 1 )
    for d = 1 : 3
        ...
        for p = 1 : nPh
            for l = 1 : size( W_f{ h, d }{ p, 1 }, 1 )
                W_f{ h, d }{ p, 1 }{ l, 1 } = gather( W_f{ h, d }{ p, 1 }{ l, 1 } );
                b_f{ h, d }{ p, 1 }{ l, 1 } = gather( b_f{ h, d }{ p, 1 }{ l, 1 } );
            end
        end
    end
end
for k = 1 : 2
    LS{ k, 1 } = gather( LS{ k, 1 } );
end
